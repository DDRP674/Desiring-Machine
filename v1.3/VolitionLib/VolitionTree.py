...
def get_exactly_n_level_nodes(node, n: int) -> list:
    """获取恰好距离当前节点n层的所有节点"""
    if n < 0: return []
    if n == 0: return [node]
    result = []
    for child in node.children: result.extend(get_exactly_n_level_nodes(child, n-1))
    return result

class VolitionTree:
    def __init__(self, RootContent: str, treename="", Threshold=0.5, historyLength=3): 
    ...
    def Grow(self) -> bool: 
        ...
        with self.lock: current_id = self.current.name 
        First2Volitions = [i["content"] for i in self.getKData(2)]
        JsonData = self.llms.GenerateVolitionsTest(First2Volitions).get("content", 0) # 记得改为GenerateVolitions
        ...
        try:
            ...
            with self.lock:
                if (current_id != self.current.name): 
                    ...
                    for child in subtree.children: child.parent = self.current
            return cond     
        except Exception as e:
    ...
    def GetRealValue(self) -> float|None:
        """获取真实价值，带锁"""
        with self.lock: message = self.ipc.query_latest_message(k=1, sender="UI_Desire", receiver="Public")
    ...
    def SelfEvaluate(self, targetVector, n=100) -> float: # 待完成
        """在树中随机抽取n个节点用于评估"""
    ...
    def EvaluatorUpdateCoroutine(self, with_history=True, cooldown=10) -> None:
        ...
        while True:
            ...
            if id == rootid: 
                gevent.sleep(cooldown) # 2025年11月8日，被这个bug硬控十小时
                continue 
            with self.lock: 
                if with_history:
                    ls = [i["contentVector"] for i in self.success]
                    random.shuffle(ls)
                    # 没有处理失败的记忆，大抵是因为没有做成的事情不会对未来造成影响罢
...
if __name__ == "__main__": 
    ...
    vt = VolitionTree("make me feel happier")
    vt.run()
    input()
