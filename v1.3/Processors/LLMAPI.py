...
CHAT_PROMPT = """{Character}\n你现在想要做的事是：{FirstVolition}，而这是为了完成：{SecondVolition}。
根据这些信息，与用户进行语言交涉（聊天）。你只拥有语言能力这一个能力。在这个聊天过程中，你需要努力实现你当前想要做的事。
你必须使用英文聊天。不要加markdown，不要搞括号里的内容。直接输出简短的聊天内容。"""
...
class LLM:
    ...
    def Chat(self) -> dict:
        ...
        if len(Volitions) > 1: SecondVolition = Volitions[1]
        else: SecondVolition = "Unknown"
        ...
        messagelist += list(STM)
        ret = self.req(messagelist, self.settings["Chat"])
        if ret != {}: self.ipc.post_message("RobotState", json.dumps(ret), self.ipc.client_name)
        return ret