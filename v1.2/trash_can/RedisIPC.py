...
class RedisIPCServer:
    ...
    def _handle_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.show:
            self.logger(f"处理请求: {action} -> {receiver}")
...
class RedisIPCClient:
    ...
    def query_latest(self, receiver: str):
        """查询指定频道的最新消息"""
        return self._send_request({
            "action": "query_latest",
            "receiver": receiver
    ...
    def fetch_earliest(self, receiver: str):
        """获取并删除指定频道的最早消息"""
        return self._send_request({
...
if __name__ == "__main__":
    ...
    # 客户端1
    client1 = RedisIPCClient("client1", listen=True, show=True)
    
    # 客户端2  
    client2 = RedisIPCClient("client2", listen=True, show=True)
