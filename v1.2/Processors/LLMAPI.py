...
ABANDON_PROMPT_1 = COMPLETION_PROMPT_1
ABANDON_PROMPT_2 = """现在你需要根据所给信息仔细检查并判断是否应该放弃这一件事。
你的依据是用户的反馈以及当前的情景。如果用户的反馈是正面的但你认为应该放弃，可以选择放弃。如果用户的反馈是负面的但你认为不应该放弃，也可以选择坚持。
用户的反馈：{Feedback}
如果认为应该放弃则返回True（谨慎返回True），如果认为应该坚持或者不确定则返回False，不应返回任何额外内容：
{FirstVolition}"""
...
提示：
你可以生成需要进一步细化的抽象行为留着后续细化，也可以生成具体可以执行的行为
比如，对用户说一些话，或者讲简短的故事或者笑话，
或者和用户进行一些交易，比如交易情绪价值、赞美等。你也可以使用一些激进的行为。
注意，你每次只能对用户使用语言上的简短的交互。
...
CHAT_PROMPT = """{Character}\n你的对话历史是：\n{FormattedSTM}
你的核心目标是：{FirstVolition}。基于这个目标与用户交流。你只能使用语言能力，用户无法直接与你对话。
用户的反馈是{Feedback}的。请基于此反馈，在保持核心目标不变的前提下，适当调整你的表达方式或策略。
如果收到正向反馈，可以考虑继续坚持当前的有效方式，如果你在试图让用户做什么，可以考虑让用户继续这么做；如果收到负向反馈，尝试用创新的方式重新表达，或者适当改变自己的行为。
使用英文回复，简短直接，不要使用markdown或括号注释，不要有描述性的括号，直接输出向用户对话的内容。"""
...
class LLM:
    ...
    def Completeness(self, firstVolition: str) -> bool:
        """完成度检查"""
        with self.lock:
            dic = self.ipc.query_latest_message(receiver="Public_RobotState")
    ...
    def Abandon(self, firstVolition: str) -> bool:
        """是否放弃"""
        with self.lock:
            dic = self.ipc.query_latest_message(receiver="Public_RobotState")
            TriggerState = self.ipc.query_latest_message(receiver="Public_UI_Desire")
        if not dic["messages"]: return False

        if not TriggerState["messages"]: TriggerState = 1
        else: TriggerState = TriggerState["messages"][0]["content"]
        ...
        for i in STM:
            ...
            FormattedSTM += i["content"] + "\n"
        messagelist = []
        messagelist.append({"role": "system", "content": ABANDON_PROMPT_1.format(
            Character=robotstate["Character"],
            FormattedSTM=FormattedSTM
        )})
        messagelist.append({"role": "user", "content": ABANDON_PROMPT_2.format(
            FirstVolition=firstVolition, # 这个并非来自于RobotState，而是直接由意志树给出
            # Feedback= "未知"
            Feedback= "负向" if TriggerState == -1 else "正向"
        )})
        res = self.req(messagelist, self.settings["Completion"])
        if not res: return False
        if "alse" in res["content"]: return False
    ...
    def GenerateVolitionsTest(self, First2Volitions: list) -> dict:
        """用于测试，不依赖IPC"""
        STM = []
        FormattedSTM = ""
        for i in STM:
            if i["role"] == "assistant": FormattedSTM += "你："
            else: FormattedSTM += "用户："
            FormattedSTM += i["content"] + "\n"
        messagelist = []
        messagelist.append({"role": "system", 
            "content": VOLITIONGENERATION_PROMPT_1.format(
    ...
    def Chat(self) -> dict: 
        ...
        if not TriggerState["messages"]: TriggerState = 1
        else: TriggerState = int(TriggerState["messages"][0]["content"])

        robotstate = json.loads(dic["messages"][0]["content"])
        STM = robotstate["STM"]
        Volitions = robotstate["Volition"]
        if Volitions: FirstVolition = Volitions[0]
        else: FirstVolition = "Unknown"
        logging.info(f"{func_name()}: FirstVolition: {FirstVolition}")
        FormattedSTM = ""
        # logging.info(f"{func_name()}: STM: {STM}")
        for i in STM:
            if i["role"] == "assistant": FormattedSTM += "你："
            else: FormattedSTM += "用户："
            FormattedSTM += i["content"] + "\n"

        messagelist = [{"role": "system", "content": CHAT_PROMPT.format(
            Character=robotstate["Character"],
            FormattedSTM= FormattedSTM if FormattedSTM else "当前没有对话历史",
            FirstVolition=FirstVolition,
            Feedback= "负向" if TriggerState == -1 else "正向"
        )}]
        # logging.info(f"{func_name()}: Chat Messagelist: {messagelist}")
        ret = self.req(messagelist, self.settings["Chat"])
        if ret != {}: self.ipc.post_message("RobotState_STM", json.dumps(ret), self.ipc.client_name)
        return ret