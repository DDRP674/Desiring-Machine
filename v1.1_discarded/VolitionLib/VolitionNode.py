...
# 这里只写了创建节点和使树生长的函数，剪枝（Backward）在VolitionTree
...
def CreateNode(content: str, name=None, is_abstract=True) -> Node:
    """创建一个节点"""
    data = VolitionData(evaluator.evaluate3, embedder.model.encode, content, is_abstract)
    if name == None: name = str(uuid.uuid4())
    return Node(name, data=data)

def LoadNode(JsonData) -> Node:
    """从持久化数据读取节点信息，
    \n包括name，content，contentVector，value，is_abstract，time_created
    \n不包括孩子们"""
    return Node(name=JsonData["id"], data=VolitionData(
        evaluator.evaluate3, embedder.model.encode, JsonData["content"], JsonData["is_abstract"],
...
def TreeConstructor(JsonData, fulldata=False) -> Node|None:
    """根据Json文件构建一个树，递归"""
...
if __name__ == "__main__":
    ...
    with open("backup/VolitionFullJsonDataSample.json", "r", encoding="utf-8") as f: 
        JsonData = json.load(f)
    def g(a): return
    start = time.time()
    root = TreeConstructor(JsonData, True)
